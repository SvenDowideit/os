...
func Testcmdline.Parse(t *testing.T) {
	assert := require.New(t)
	...
	}, cmdline.Parse("a b rancher.key1=value1 c rancher.key2=value2"))

	assert.Equal(map[interface{}]interface{}{
		"rancher": map[interface{}]interface{}{
			"key": "a,b",
		},
	}, cmdline.Parse("rancher.key=a,b"))
	...
	}, cmdline.Parse("rancher.keyArray=[1,2]"))

	assert.Equal(map[interface{}]interface{}{
		"rancher": map[interface{}]interface{}{
			"strArray": []interface{}{"url:http://192.168.1.100/cloud-config?a=b"},
		},
	}, cmdline.Parse("rancher.strArray=[\"url:http://192.168.1.100/cloud-config?a=b\"]"))
